#!/usr/bin/env python3
"""
QLoRA SFT training script aligned with finetune_data.py outputs.
...
def parse_args() -> argparse.Namespace:
    """Parse training, data, LoRA/QLoRA, and output-path arguments."""
    parser = argparse.ArgumentParser(description="Train Gemma-style LoRA/QLoRA with messages-format JSONL")

    # Paths
    parser.add_argument("--model_path", type=str, default="/projects/checkpoint/gemma-3-4b-it")
    parser.add_argument("--train_file", type=str, default="/projects/checkpoint/datasets/second/train_stage1.jsonl")
    parser.add_argument("--validation_file", type=str, default="/projects/checkpoint/datasets/second/val.jsonl")
    parser.add_argument("--output_dir", type=str, default="/projects/checkpoint/gemma-3-4b-med-lora/gemma-3-4b-med-lora-newdata-r16")
    ...
    # LoRA / QLoRA
    parser.add_argument("--use_4bit", action="store_true", help="Enable QLoRA 4-bit loading.")
...
def get_torch_dtype(name: str) -> torch.dtype:
    """Convert a dtype name from CLI arguments into a PyTorch dtype."""
    mapping = {
...
def load_jsonl_dataset(train_file: str, validation_file: str | None, seed: int):
    """Load JSONL splits and shuffle available train/validation splits with a fixed seed."""
...
def validate_messages(example: dict[str, Any]) -> bool:
    """Filter rows whose messages are missing, malformed, or empty."""
...
def main() -> None:
    ...
    # QLoRA uses bitsandbytes 4-bit loading; plain LoRA follows the requested fp/bf dtype.
    ...
    # Keep target modules CLI-configurable for compatibility with related model architectures.
    ...
    if args.adapter_path:
        # Continue training an existing adapter instead of asking SFTTrainer to create a new one.
        LOGGER.info("Loading existing LoRA adapter from %s", args.adapter_path)
    ...
    assistant_only_loss = (
        args.assistant_only_loss and not args.force_disable_assistant_only_loss
        )

    if assistant_only_loss:
        # Let the installed TRL version handle chat-template masking support.
        LOGGER.info(
        "assistant_only_loss requested. Letting SFTTrainer handle chat-template masking."
        )
